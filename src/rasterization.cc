...
//   1. The texel being visited (coordinates correspond to its top-left corner in image space).
//   2. The center of the texel being visited, in texture space.
void walk (const AABB2& box, int width, int height, const std::function<void(const Texel&, const vec2&)>& visit)
{
    ...
    // Walk over the AABB's texels and evaluate the visit function on their centers.
    ...
    for (p.y = bmin.y; p.y < bmax.y; ++p.y)
    {
        ...
        {
            vec2 texel_center = texture_coordinates(p, width, height) + offset_to_center;
...
std::vector<Texel> kx::rasterize_triangle_contained (const Triangle2& triangle, int width, int height)
{
    ...
    walk(box, width, height, [&] (const Texel& p, const vec2& texel_center) {
        if (contains(triangle, texel_center))
            points.push_back(p);
    });
...
std::vector<Texel> kx::rasterize_quad_contained (const Quad2& quad, int width, int height)
{
    // Construct the AABB around the triangle.
    AABB2 box;
    box.add(quad.p0);
...
std::vector<Texel> kx::rasterize_aabb_overlapping (const AABB2& box, int width, int height)
{
    ...
    std::vector<Texel> points;
    walk(_box, width, height, [&] (const Texel& p, const vec2&) {
        points.push_back(p);
    });
