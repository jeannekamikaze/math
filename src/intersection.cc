...
KX_MATH_API PlaneIntersection kx::intersect(const Plane& p,
                                            const Triangle3& t) {
  ...
  switch (s) {
    ...
    default:
      return s > 0 ? PlaneIntersection::front
                   : (s < 0 ? PlaneIntersection::back
                            : PlaneIntersection::containing);
  }
}

KX_MATH_API PlaneIntersection kx::intersect(const AxisPlane& p,
                                            const Triangle3& t) {
  int s0 = sgn0(distanceTo(p, t.p0));
  int s1 = sgn0(distanceTo(p, t.p1));
  int s2 = sgn0(distanceTo(p, t.p2));
...
KX_MATH_API VolumeIntersection kx::intersect(const Frustum& f,
                                             const AABB3& box) {
  bool is_intersecting = false;
  PlaneIntersection i;

  i = intersect(f.left, box);
  if (i == PlaneIntersection::back)
    return VolumeIntersection::outside;
  else if (i == PlaneIntersection::intersecting)
  ...
  if (is_intersecting)
    return VolumeIntersection::intersecting;
  else
    return VolumeIntersection::inside;
}
...
KX_MATH_API bool kx::intersect(const Ray3& r, const AABB3& a, R& tmin,
                               R& tmax) {
  tmin = R_MIN;
  tmax = R_MAX;
  if (r.dir.x != 0.0f) {
...
KX_MATH_API bool kx::intersect(const Ray3& r, const Sphere& s, R& tmin,
                               R& tmax) {
...
KX_MATH_API bool kx::intersect(const Ray3& r, const Triangle3& tri, R& t, R eps,
                               R tau) {
  return intersect(r, tri.p0, tri.p1, tri.p2, t, eps, tau);
}

KX_MATH_API bool kx::intersect(const Ray3& r, const vec3& p0, const vec3& p1,
                               const vec3& p2, R& t, R eps, R tau) {
  vec3 a = p0 - p2;
  vec3 b = p1 - p2;
  vec3 c = r.pos - p2;
...
KX_MATH_API bool kx::contains(const vec2& p0, const vec2& p1, const vec2& p2,
                              const vec2& p) {
  vec2 v1 = p1 - p0;
  vec2 v2 = p2 - p0;
  R d = det(v1, v2);
...
KX_MATH_API bool kx::contains(const vec2& p0, const vec2& p1, const vec2&,
