...
// Conventions:
//
// - The input geometry's points must be in normalised textured coordinates: [0,1]
//
// - The origin in normalised texture coordinates (input) is in the bottom-left corner.
//
...
std::vector<Texel> rasterize_triangle_contained (const Triangle2&, int width, int height);

/// Rasterize the quad using the barycentric algorithm.
std::vector<Texel> rasterize_quad_contained (const Quad2&, int width, int height);

/// Rasterize the AABB.
std::vector<Texel> rasterize_aabb_overlapping (const AABB2&, int width, int height);
